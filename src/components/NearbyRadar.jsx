      ...
      processedWaypoints.forEach((wp) => {
        ...
        const popupContent = `
          <div style="color: #0a192f; font-family: sans-serif; font-size: 12px; width: 180px; line-height: 1.4;">
            ...
            <strong style="color: ${getContrastColor(markerColor)}; display: block; margin-bottom: 4px; font-size: 11px;">${displayLabel}${isInventoried ? ' (Inventoried)' : ''}</strong>
            ${wp.desc ? `<div style="margin-top: 4px; color: #555;">${wp.desc}</div>` : ''}
            ${ageDays !== null ? `<div style="margin-top: 4px; color: ${getContrastColor(markerColor)}; font-weight: bold;">Age: ${ageDays} days</div>` : ''}
